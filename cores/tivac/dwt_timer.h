...
typedef volatile uint32_t * const cuint32_ptr;
...
void dwt_delay_cycles(const uint32_t cycles)
{
    const uint32_t start = CYCCNT_;
    do {
    } while ((CYCCNT_ - start) < cycles);
}
...
#define usec2cycles(n) ((n*(F_CPU/1000000))-32) /* cycle overhead for dwt polling loop overhead */
